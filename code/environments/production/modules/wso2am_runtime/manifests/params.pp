...
class wso2am_runtime::params {
  ...
  if str2bool("$::use_hieradata") {
    ...
    #new
    $key_validator            = hiera('wso2::key_validator')
    $throttling               = hiera('wso2::throttling')
    $auth_manager             = hiera('wso2::auth_manager')
    $oauth_configurations     = hiera('wso2::oauth_configurations')
    $api_store                = hiera('wso2::api_store')
